 ...
 */
static void
tty_const(struct tty_SOS *tty_init)
{
	uint_t count;
	 ...
	 * Initialize data.
	 */
	heap_alloc(20, tty_init.tty_que_buf);

	tty_init->tty_num_chars=0;
	tty_init->tty_cursor = 0; // initial cursor position
	/* initial number of lines */
	tty_init->tty_cur_line=0;
	tty_init->tty_cur_line_scr=0;
	/* XXX assume a color terminal */
	tty_init->tty_type = COLOR;
	tty_init->tty_key_holds=0;
	tty_init->tty_ptr_tail_que = tty_init.tty_que_buf;
	/* initial position in VRAM */
	tty_init->tty_cur_pos = 0;
	if (tty_init->tty_type == COLOR) {
		tty_init->tty_base_vram = COLOR_VRAM;
		tty_init->tty_cur_addr = (byte *)COLOR_VRAM;
	}
	else {
		tty_init->tty_base_vram = MONO_VRAM;
		tty_init->tty_cur_addr = (byte *)MONO_VRAM;
	}

	/*
	 * Initialize functions.
	 */
	tty_init->tty_init = vram_init;
	tty_init->tty_write = vram_write;
	tty_init->tty_unwrite = term_unwrite;
	tty_init->tty_inter_spec = inter_spec;
	tty_init->tty_que_char = queue_char;
	tty_init->tty_clear_screen = clear_screen;
	tty_init->tty_clear_line = clear_line;
	tty_init->tty_updown = up_down;
}
 ...
 */
void
term_init(struct tty_SOS *tty_init)
{

	/* allocate terminal */
	heap_alloc(sizeof(struct tty_SOS), tty_init);
