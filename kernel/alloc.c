 ...
 */
void
kfree_page(struct mem_abs *free_addr)
{
	...
	for (ptr_heap=ptr_heaps[0]; ptr_heap->mem_addr!=(uaddr_t)free_addr &&
 ...
 */
void *
kmalloc_page(uint_t alloc_size)
{
	uint_t m, i;
	struct mem_abs *ptr_heap;
	struct mem_abs *ptr_heap1;
	...
	/* we have a block that suits the request */
		...
		if (ptr_heap->mem_size>alloc_size) {
			...
			ptr_byte = (byte *)addr_tmp; // allocation address
			...
			if (heaps_first[0] == 0) {
				heaps_first[0]=1;
				heap_sizes[0]=ptr_heap->mem_size;
			}
